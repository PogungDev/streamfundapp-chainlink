    ...
    const interval = setInterval(() => {
      setYieldData((prev) =>
        prev.map((vault) => {
          ...
          setAnimations((prev) => ({
            ...prev,
            [vault.creator]: `📈 Yield increased +${increase.toFixed(2)}%`,
          }))
          ...
          setTimeout(() => {
            setAnimations((prev) => {
              ...
              delete newAnimations[vault.creator]
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <span className="font-mono text-sm">{isActive ? "🟢 Auto-updating every 10s" : "⚪ Paused"}</span>
            </div>
            <Button
        ...
        {yieldData.map((vault) => (
          <Card key={vault.creator} className="glass rounded-2xl border-pink-500/20 relative overflow-hidden">
            ...
            <CardContent>
              ...
              {/* Animation Overlay */}
              {animations[vault.creator] && (
                <div className="absolute top-2 right-2 bg-emerald-500/20 text-emerald-400 px-3 py-1 rounded-full text-xs font-mono animate-bounce">
                  {animations[vault.creator]}
                </div>
              )}
            </CardContent>
